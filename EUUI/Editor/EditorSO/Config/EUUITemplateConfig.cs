...
namespace EUFramework.Extension.EUUI.Editor
{
    ...
    public class EUUITemplateConfig : ScriptableObject
    {
        ...
        [Tooltip("IController 的完全限定类型名（分部类继承与扩展方法约束）。\n" +
                 "- 留空：使用历史路径 `EUFramework.Core.MVC.Interface.IController` / `IArchitecture`。\n" +
                 "- 填写示例：`EUFramework.Core.IController`，`GetArchitecture()` 返回类型自动为同命名空间下的 `IArchitecture`。")]
        public string mvcControllerInterfaceQualifiedName = "";
